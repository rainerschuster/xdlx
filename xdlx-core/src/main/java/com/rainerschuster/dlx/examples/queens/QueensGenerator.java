...
public class QueensGenerator extends
  ...
  public List<QueensColumnValue> generatePrimaryColumnValues() {
    ...
    List<QueensColumnValue> primaryColumnValues = new ArrayList<QueensColumnValue>(n + n);
    for (int j = 0; j < n; j++) {
      //int t = j; // primitive column order
      int t = ((j % 2 == 1) ? n - 1 - j : n + j) >> 1; // optimized column order
  ...
  public List<QueensColumnValue> generateSecondaryColumnValues() {
    ...
    List<QueensColumnValue> secondaryColumnValues = new ArrayList<QueensColumnValue>(nn - 1);
    for (int j = 1; j < nn; j++) {
      QueensColumnValue columnValue = new QueensColumnValue(QueensEnum.class);
  ...
  public List<QueensValue> generateValues() {
    int n = properties.getN();
    List<QueensValue> values = new ArrayList<QueensValue>(n * n);
    for (int j = 0; j < n; j++) {
      for (int k = 0; k < n; k++) {
