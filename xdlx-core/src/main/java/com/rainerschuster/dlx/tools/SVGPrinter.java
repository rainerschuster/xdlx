...
public class SVGPrinter<C, V extends Value<C>> {
    ...
    public void print(final boolean[][] matrix, final boolean[] columnArray) throws IOException {
        // Convert data structure to matrix
        /*final List<C> list = new ArrayList<C>();
        for (Column<C, V> column : dlData) {
            list.add(column.getValue());
        }*/
        final List<Column<C, V>> list = dlData.getAllColumns();
        ...
        final Font f = new Font("Dialog", Font.BOLD, 14);
        g.setFont(f);
        ...
        for (int column = 0; column < list.size(); column++) {
            ...
            g.setPaint(Color.BLACK);
            final QueensColumnValue cv = (QueensColumnValue) myColumn.getValue();
            ...
            if (columnArray[column] || (previousColumns == null || previousColumns[column])) {
                ...
                if (myColumn.isPrimary()) {
                    ...
                    g.fillPolygon(
                            new int[] { startHorizontal - 8, startHorizontal - 8, startHorizontal },
                            new int[] {4 + 48, 12 + 48, 8 + 48 }, 3);
                    // g.setPaint((myColumn.getNext().getPrev() == myColumn) ?
                    // Color.BLUE : Color.RED);
                    g.setPaint(Color.BLUE);
                    g.drawLine(cLastHorizontal, 48 + 40, startHorizontal + 8, 48 + 40);
                    g.fillPolygon(
                            new int[] { cLastHorizontal + 8, cLastHorizontal + 8, cLastHorizontal },
                            new int[] {36 + 48, 44 + 48, 40 + 48 }, 3);

                    cLastHorizontal = startHorizontal + 32;
                }
                g.setPaint(Color.BLUE);
                g.fillPolygon(
                        new int[] { startHorizontal + 4, startHorizontal + 12, startHorizontal + 8 },
                        new int[] {96 + 8, 96 + 8, 96 }, 3);
        ...
        for (int row = 0; row < matrix.length; row++) {
            final boolean[] myrow = matrix[row];
            final int startVertical = ((row + 2) * 48) + 16;
            int lastHorizontal = 48;
            final QueensValue value = (QueensValue) dlData.getGeneratedValues().get(row);
            // Print chess field
            g.setPaint(Color.BLACK);
            ...
            g.setPaint(Color.BLACK);
            final int qr = value.getRow();
            final int qc = value.getColumn();
            ...
            for (int column = 0; column < myrow.length; column++) {
                if (myrow[column]) {
                    empty = false;
                    final int startHorizontal = (column + 2) * 48;
                    // Draw node
                    ...
                    g.fillPolygon(
                            new int[] { startHorizontal - 8, startHorizontal - 8, startHorizontal },
                            new int[] {startVertical + 4, startVertical + 12, startVertical + 8 }, 3);
                    g.drawLine(lastHorizontal + (found ? 8 : 0), startVertical + 24, startHorizontal + 8, startVertical + 24);
                    g.fillPolygon(
                            new int[] { startHorizontal + 40, startHorizontal + 40, startHorizontal + 32 },
                            new int[] { startVertical + 20, startVertical + 28, startVertical + 24 }, 3);
            ...
            if (columnArray[column] || (previousColumns == null || previousColumns[column])) {
                ...
                for (int row = 0; row < matrix.length; row++) {
                    ...
                    if (myrow[column]) {
                        ...
                        g.fillPolygon(
                                new int[] { startHorizontal + 4, startHorizontal + 12, startHorizontal + 8 },
                                new int[] { startVertical + 40, startVertical + 40, startVertical + 32 }, 3);
                        g.drawLine(startHorizontal + 24, lastVertical, startHorizontal + 24, startVertical);
                        g.fillPolygon(
                                new int[] { startHorizontal + 20, startHorizontal + 28, startHorizontal + 24 },
                                new int[] { startVertical - 8, startVertical - 8, startVertical }, 3);
        ...
        final Writer out = new FileWriter("graph" + (count < 10 ? "0" : "") + count + ".svg");
        g.stream(out, useCSS);
    ...
    public static void main(String[] args) {
        ...
        /*dl.addSolutionListener(new SolutionListener<QueensColumnValue, QueensValue>() {
            ...
            public void onSolution(final int count, final int level, final List<Node<EnumMap<QueensEnum, Integer>>> solution) {
...
//        });
    ...
    /**
     * Transforms the data structure to a two-dimensional (boolean) matrix. NOTE
     * Remind setting the previous matrix!
     */
    public boolean[][] getMatrix() {
        final List<Column<C, V>> list = dlData.getAllColumns();
        final boolean[][] matrix = new boolean[dlData.getGeneratedValues().size()][list.size()];
        dlData.resetMarks();
        for (Column<C, V> column : dlData) {
            for (Node<C, V> row : column) {
                if (!row.isMarked()) {
                    final boolean[] myrow = new boolean[list.size()];
                    for (Node<C, V> node : row) {
                        final int index = list.indexOf(node.getColumn());
                        myrow[index] = true;
                        node.setMarked(true);
                    }
    ...
    @Deprecated
    private int find(final List<boolean[]> list, final boolean[] element) {
        if (list != null) {
            for (int i = 0; i < list.size(); i++) {
                final boolean[] listItem = list.get(i);
