...
// TODO QUEST moegliche Condition ist, dass mindestens eine PrimaryColumn
// vorkommen muss, damit es als Inhalt zaehlt
public class ColumnCollection<C, V extends Value<C>> implements Collection<Column<C, V>> {
...
public boolean add(Column<C, V> column) {
    if (column instanceof PrimaryColumn) {
      PrimaryColumn<C, V> myColumn = (PrimaryColumn<C, V>) column;
      if (root == null) {
        root = myColumn;
      } else {
...
public boolean containsAll(Collection<?> arg0) {
  ...
  public int indexOf(Object o) {
    int index = 0;
    for (Iterator<PrimaryColumn<C, V>> iter = primaryColumnIterator(); iter
        .hasNext();) {
...
public boolean remove(Object o) {
    for (Iterator<PrimaryColumn<C, V>> iter = primaryColumnIterator(); iter
        .hasNext();) {
      PrimaryColumn<C, V> column = iter.next();
